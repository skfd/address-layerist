...
# A stable reference point (Toronto City Hall) -- the math is location-agnostic.
REF_LON, REF_LAT = -79.3839, 43.6534
...
def test_pixel_offsets_within_tile():
    for zoom in (12, 14, 16, 18):
        _, _, ox, oy = pixel_in_tile(REF_LON, REF_LAT, zoom)
...
def test_tile_contains_its_point():
    for zoom in (12, 14, 16, 18):
        tx, ty = lonlat_to_tile(REF_LON, REF_LAT, zoom)
        west, south, east, north = tile_bounds(tx, ty, zoom)
        assert west <= REF_LON <= east
        assert south <= REF_LAT <= north


def test_pixel_and_tile_offset_agree():
    for zoom in (12, 16, 18):
        x, y = lonlat_to_pixel(REF_LON, REF_LAT, zoom)
        tx, ty, ox, oy = pixel_in_tile(REF_LON, REF_LAT, zoom)
