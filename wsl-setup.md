...
It should print a version (2.49 or newer).

## Notes

- The engine reads `wsl_distro` from `[layer]` in `layer.toml` (default
  `Ubuntu`) to pick which distro to drive.
- Building from source is only needed if the packaged tippecanoe is too old:
  `sudo apt-get install -y build-essential libsqlite3-dev zlib1g-dev git`, then
  `git clone https://github.com/felt/tippecanoe.git && cd tippecanoe && make -j
  && sudo make install`.
